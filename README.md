...
- **Subpath** (`https://racheloalden.com/monalease/`): `npm run build:site` — sets Vite `base` to `/monalease/`. **nginx** handles SPA deep links — see below.
...
4. Add a **`location`** so refreshes and direct links to `/monalease/room/...` return `index.html`. Example if files live under `/var/www/html/monalease/`:
...
5. Open `https://racheloalden.com/monalease/`, join a room, then refresh the room URL to confirm the fallback works.
